...
**Backend MVP** is built and green in `gathr-backend/`. 123 tests pass across 10 crates; `cargo fmt --check`,
`clippy --workspace --all-targets -D warnings`, and `sqlx prepare --check` are all clean.

**iOS app** is built and runnable in `gathr-ios/` — 57 unit tests across four SPM packages
(`Models` 8, `DesignSystem` 11, `Networking` 8, `Features` 30) plus a UI test that drives the real
app against the live backend, all in Swift 6 language mode with complete strict concurrency on the
iOS 26.4 simulator. `DesignSystem` implements the Section 7 token set and component inventory;
`Features` ships Onboarding, Home, Event Detail with the RSVP sheet, Create, Join and Profile,
driven by `@Observable` models over protocol-injected services. The app target is generated by
XcodeGen from `project.yml`, with the generated `Gathr.xcodeproj` committed so the project opens
without the tool installed.

`GathrUITests/DemoFlowUITests` is the executable version of Section 18's demo: it walks all three
onboarding pages, enters a name, publishes an event, lands on the detail screen with a live
countdown, navigates back, and asserts the event is on Home — against a real Postgres and a real API. It caught five defects that
unit tests could not: `HomeModel` being reconstructed on every render, Home not refreshing after a
publish, the FAB drawing over pushed screens, the event-detail action bar clipping behind the tab
bar, and a `NavigationStack` binding whose setter was a no-op so system back never updated the
router.

Onboarding (8.2) is a single animated screen — a seamless marquee of all five pieces of artwork with
self-cycling copy, ending in Get Started, which presents S2 as a sheet. Reduce Motion disables the
marquee, the bob, and the copy cycle together. The old `hasOnboarded` flag is gone: onboarding *is*
the signed-out screen. `Features` exposes
`Onboarding` as its own module and owns the image catalog; `AppShell` no longer knows onboarding
exists. The screen is a paged `ScrollView` rather than `TabView(.page)` for the reasons in 8.2, and
its layout metrics live in `DesignSystem` (`OnboardingMetrics`, `Palette.photoVeil`,
`Palette.photoClarityMask`, `VeiledPhoto`) so no spacing or colour literal sits in the feature.
A test asserts every referenced image resolves in the module bundle, which is the failure mode a
renamed asset would otherwise produce only as a blank screen at runtime.

**The phone and email code flow is live end to end.** `POST /v1/auth/otp/request` and
`/verify` are backed by an `otp_challenges` table, and eight integration tests cover the paths that
matter: a code signs its owner in, the same address in a different case reaches the same account,
five wrong attempts lock the challenge, reissuing retires the previous code, and a malformed
destination is refused before a code is ever issued. A second UI test drives the whole thing on
device — Get Started, Continue with Email, type an address, read the code, land on Home.

Code **delivery** is the piece that is stubbed: with no SMS or email provider chosen (14.2),
`/otp/request` answers `provider_unavailable` unless `GATHR_ALLOW_DEV_AUTH` is set, in which case it
returns the code in the response and the app prints it under the code field. That is what makes the
flow demoable today and it must not reach a real build.

**Provider sign-in (D11, 8.12) is built on both sides but not yet activated.** `crates/infra_oidc`
verifies Apple and Google ID tokens against the provider JWKS — RS256 signature, issuer, audience,
expiry with an explicit 30s skew allowance, and the per-provider nonce transform — behind a cached
key fetch. Seven unit tests cover it using a fixture RSA key: a valid token yields subject/name/email,
and tokens minted for another audience, expired, presented under the wrong provider's issuer, signed
by an unknown key, or carrying another attempt's nonce are all rejected. `POST /v1/auth/oauth`
finds-or-creates the user against a new `identities` table keyed on `(provider, subject)`, and
`PATCH /v1/me` renames. On iOS, `SignIn` implements the Apple button and the Google PKCE flow through
`ASWebAuthenticationSession`, and S11 gained the rename form.

What is missing is **credentials, not code**: an Apple Developer Team ID with the Sign in with Apple
capability on `app.gathr.ios`, and a Google Cloud OAuth **iOS** client ID. Set them as
`GATHR_APPLE_AUDIENCES` / `GATHR_GOOGLE_AUDIENCES` on the server and `GathrGoogleClientID` in
`project.yml`. Until then `/v1/auth/oauth` answers `provider_unavailable` (503) by design, the Google
button hides itself, and the development-only name field carries the demo. The Sign in with Apple
entitlement is already declared in `project.yml` and does not break simulator builds.

Auth on device uses `SessionStore`, a Keychain-backed actor. Note that simulator keychain state
survives app uninstall — `xcrun simctl keychain booted reset` is required between clean-state runs,
which is why the UI test's first execution appeared to skip sign-in.

The feed endpoint was extended during the iOS build: `EventSummary` now carries `going_guests` and
`preview_guest_names` (a lateral join over the first four going guests), because the hero card in
the mockup renders an avatar cluster and "+14 going" and the original feed shape could not supply
either. Section 5.4's cluster arithmetic is now backed by real data at the feed level.

**Images are live on Cloudinary** (D-media supersedes the R2 plan). `POST /v1/media/sign` returns a
SHA-1 upload ticket, the phone uploads straight to Cloudinary, `POST /v1/media` records what came
back, and `PUT /v1/events/{id}/cover/{media_id}` attaches it — answering the transformed delivery
URL (`c_fill,w_1200,h_800,q_auto,f_auto`). The **API secret never leaves the server**, asserted by a
test rather than by care. Verified end to end against the real account: a signature this server
generated was accepted by Cloudinary, and the transformed URL served a 200. A host can only attach
uploads they recorded, and only to events they own — both 403/404, both tested.

**Email delivery is live on Resend.** With `GATHR_ALLOW_DEV_AUTH=false` the OTP endpoint sends the
code instead of returning it; a send failure surfaces as `delivery_failed` and stores nothing
redeemable, so nobody is left holding a code that was never delivered.

**Notifications are built end to end except the certificate.** `POST /v1/devices` upserts on the
APNs token so re-launching keeps one row per handset; publishing an event queues `day_before` and
`hours_before` jobs, skipping any whose moment has already passed; cancelling calls them off. The
**`gathr-worker` binary** drains `reminder_jobs` with `FOR UPDATE SKIP LOCKED`, so two workers can
run without double-sending — proven by a test that sweeps twice and asserts the second sweep sends
nothing. Muting silences an event without unregistering the device. APNs itself sits behind a
`Dispatcher` trait: with no `.p8` key configured the worker logs what it would have sent, which is
why the whole path is testable today. Add `APNS_TEAM_ID`, `APNS_KEY_ID`, `APNS_PRIVATE_KEY` and
`APNS_TOPIC` and it delivers.

**Messaging is live, including realtime.** `gathr-domain::message` owns the content rules (trim,
2 000 *characters* not bytes, and the host-only vs participant posting right), sequence numbers come
from the 4.6 upsert so the first message in an event is seq 1, and `POST /v1/events/{id}/messages`
broadcasts to `EventHub` subscribers as it commits. Five integration tests cover the thread — a
guest who RSVPed can read and reply while a stranger gets 403 on both, paging by `after_seq` returns
only the unseen tail, a rejected message never burns a sequence number, and a replayed
Idempotency-Key returns the original message rather than posting twice. Two more tests drive a
**real WebSocket**: an authorized reader opens `/v1/events/{id}/stream`, a message posted over HTTP
arrives on the socket as a JSON frame, and an uninvited reader's upgrade is refused.

**Rate limiting** is an actix `from_fn` middleware with per-caller fixed windows in five buckets,
tightest where it matters: issuing verification codes (8 per 15 min) is scarcer than invite
resolution (20/min, the enumeration surface of 13.1), which is scarcer than ordinary reads
(300/min). Exhausting one bucket does not lock a caller out of the rest of the API, and a refusal
carries `Retry-After`. It is **in-process**, so a multi-instance deploy needs a shared store —
Redis or Postgres — before the numbers mean anything.

**`api/gathr.postman_collection.json`** covers all 24 declared routes. Import it, set `{{baseUrl}}`,
run *Auth → Dev sign-in*, and the collection captures the token pair, event id and invite code into
variables for everything downstream; mutating requests send a fresh `{{$guid}}` as the
Idempotency-Key automatically. A check in the build notes verifies the collection against the route
table so it cannot silently drift.

**Still missing on the backend**: an SMS provider (14.2 — phone sign-in is dead without it), the
`/v1/sync` delta endpoint, check-in, guest-history claim, the public invite projection, and the
Section 13 privacy surface (reports, blocks, account deletion, data export). On iOS, the two steps
that follow verification — **Your Profile** (avatar, name, bio) and **Get Notified** — are not built;
verified users land on Home with a placeholder name derived from their email, editable in S11.

**Working end to end**: dev auth with JWT access tokens plus refresh rotation and family revocation
on reuse · event create/publish/cancel with server-side validation · Crockford invite codes with
collision retry · the server-rendered web invite page with Open Graph tags · guest RSVP with no
account and no app, backed by shadow users and a signed guest-session cookie · the capacity
transaction under `SELECT … FOR UPDATE` · opt-in waitlist and host promotion · guest list separating
people from seats · this_week/hosting/attending feeds with the host-priority boost · idempotency
with request fingerprinting so a replayed key with a different payload conflicts rather than lying.

**Verified, not assumed**: `crates/application/tests/capacity.rs` fires 20 concurrent RSVPs at a
capacity-5 event and asserts exactly 5 are admitted and stored seats equal capacity — the race in
Section 4.6 is closed, not just guarded on paper.

**Deliberately not built yet**: media upload and presign (needs R2 credentials; the invite page uses
a category-tinted gradient in the meantime), Sign in with Apple and phone OTP (Section 14.2 must be
resolved first — `/v1/auth/dev` is the stand-in and is config-gated off by default), APNs and the
reminder worker, chat, and the L0 platform items from Section 24, which belong to a product build
rather than a demo build (Section 18).

**Known gaps against this document**: `X-Request-Id` is set on error responses only, not on every
response as Section 12.1 requires — a small middleware closes it. Feeds return whole lists rather
than keyset pages; fine at demo scale, not at Section 12.1's limits. The `application` crate depends
on `infra_db` directly rather than through ports; the domain crate is pure as CLAUDE.md requires,
but the hexagon is not enforced at the application boundary.
